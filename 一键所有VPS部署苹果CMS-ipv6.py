...
servers = [
    ...
    {"name": "美国", "hostname": "2602:294:0:b7:1234:1234:f1a2:0001", "port": 22, "username": "root", "password": "qwe8520", "domain": "mac.22222222.best"}   
...
def update_server(name, hostname, port, username, password, domain):
    try:
        ...
        print(f"{name} 申请证书")
        stdin, stdout, stderr = client.exec_command("curl https://get.acme.sh | sh && ~/.acme.sh/acme.sh --register-account -m xxxx@gmail.com && ~/.acme.sh/acme.sh --issue -d {} --standalone --listen-v6".format(domain))
        print(f"正在申请中:")
        while not stdout.channel.exit_status_ready():
            if stdout.channel.recv_ready():
                print(stdout.channel.recv(1024).decode(), end="")
